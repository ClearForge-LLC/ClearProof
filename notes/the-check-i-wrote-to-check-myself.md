...
- **A commit on a branch I wasn't on.** A setup line switched to the main branch before creating a
  ...
  nothing like its cause. This third one is a shell-chain instance of a rule this repo already has —
  [a returned failure that every caller discards is a silent failure](the-answer-nobody-read.md).
  What makes it belong here too is *who wrote the caller*: the discarding code was written seconds
  earlier, by the same agent, for the sole purpose of getting this job done.
...
## Where this sits next to the neighbouring rule

[A Returned Failure That Every Caller Discards Is a Silent Failure](the-answer-nobody-read.md) is
about durable code: a function reports a problem correctly and its callers throw the answer away.
The fix there is mutation testing and checked return values, because that code is reviewed, tested
and long-lived.

This note is about the opposite kind of code — the ten lines written inline to check one result,
which will be run once and deleted. It is never reviewed, never tested, and *it is the arbiter*. The
two rules meet at the same symptom and separate at the remedy: one is fixed by testing the code you
keep, the other by distrusting the code you don't.
